...
const PASSIVE_SKILLS = [
  { id: 'dodge', emoji: '💨', name: 'Dodge', desc: 'Passive ประจำตัว ใช้หลบกระสุนได้ 1 ครั้ง เมื่อทำงานแล้วสกิลจะหายไป' }
];
const ACTIVE_SKILLS = [
...
function initModelPreview() {
  ...
  previewCamera.position.set(0, 0.86, 2.75);
...
function fitModelPreviewToFrame(force = false) {
  ...
  previewRoot.scale.setScalar(PREVIEW_MODEL_BOOST);
  ...
  if (!isFinite(size.y) || size.y <= 0.01) {
    box = new THREE.Box3().setFromObject(previewRoot);
    box.getSize(size);
    box.getCenter(center);
  }
  if (!isFinite(size.y) || size.y <= 0.01) {
  ...
  if (data.state === 'lobby') {
    ...
    data.players.forEach(p => {
      if (p.id === selfId) {
        ...
        selfAlive = true;
        myPassiveSkill = p.dodgeUsed ? null : (p.passiveSkill || null);
        myDodgeUsed = !!p.dodgeUsed;
...
function renderSkillPanel(players = []) {
  ...
  players.forEach(p => {
    const passive = (p.passiveSkill && !p.dodgeUsed) ? passiveById(p.passiveSkill) : null;
    ...
    html += `<div class="skillRow" data-id="${p.id}">
      ...
      <span class="skillIcon ${passive ? '' : 'empty'}" title="${passive ? passive.name + ' — ' + passive.desc : (p.dodgeUsed ? 'Dodge ถูกใช้ไปแล้ว' : 'ไม่มี Passive')}">${passive ? passive.emoji : '–'}</span>
...
socket.on('skillState', data => {
  ...
  if (me) {
    myPassiveSkill = me.dodgeUsed ? null : (me.passiveSkill || null);
    myDodgeUsed = !!me.dodgeUsed;
...
socket.on('yourSkillState', data => {
  myPassiveSkill = data.dodgeUsed ? null : (data.passiveSkill || null);
  myDodgeUsed = !!data.dodgeUsed;
...
function spawnShieldBubble(entry, color = 0x9fd3ff, duration = 1.15) {
  const geo = new THREE.SphereGeometry(0.76 * (entry.size || 1), 32, 18);
  const mat = new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.42, wireframe: true, depthWrite: false });
  const mesh = new THREE.Mesh(geo, mat);
  mesh.position.set(entry.x, 0.78 * (entry.size || 1), entry.z);
  scene.add(mesh);
  fxBubbles.push({ mesh, life: 0, duration, base: 1 });
  spawnFlash(entry.x, 1.05 * (entry.size || 1), entry.z, 1.15 * (entry.size || 1), color, 0.38);
}
...
socket.on('roundStart', data => {
  ...
  if (spectating) {
    ...
    myPassiveSkill = me ? (me.dodgeUsed ? null : me.passiveSkill) : myPassiveSkill;
    myDodgeUsed = !!(me && me.dodgeUsed);
